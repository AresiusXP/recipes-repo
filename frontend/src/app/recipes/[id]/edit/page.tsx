...
export default function EditRecipePage(props: { params: Promise<{ id: string }> }) {
  return (
    <div className="mx-auto max-w-2xl overflow-hidden rounded-3xl border border-zinc-200/80 bg-white/90 p-6 shadow-sm backdrop-blur-sm dark:border-zinc-800/80 dark:bg-zinc-800/80 sm:p-10">
      ...
      <Suspense fallback={<RecipeFormSkeleton />}>
        <EditRecipeContent params={props.params} />
      </Suspense>
    </div>
  );
}

async function EditRecipeContent({ params }: { params: Promise<{ id: string }> }) {
