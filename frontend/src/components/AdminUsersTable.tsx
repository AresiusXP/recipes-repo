...
export function AdminUsersTable({ users, currentUserId }: AdminUsersTableProps) {
  ...
  async function handleBan(userId: string) {
    ...
    setPending(null);
    if (!result.success) {
      showToast("error", result.error ?? "Failed to ban user.");
      return;
    }
    router.refresh();
  }
  ...
  async function handleUnban(userId: string) {
    ...
    setPending(null);
    if (!result.success) {
      showToast("error", result.error ?? "Failed to unban user.");
      return;
    }
    router.refresh();
  }
  ...
  return (
    <div className="space-y-4">
      <ConfirmDialog
        ...
        title="Ban user?"
        description="The user will be immediately signed out and blocked from logging in. You can unban them at any time."
        confirmLabel="Ban"
        confirmingLabel="Banning…"
        isConfirming={pending === confirmBan}
        tone="warning"
      ...
      <ConfirmDialog
        ...
        title="Delete user?"
        description="This will permanently delete the user and all their data (recipes, sessions, notifications). This action cannot be undone."
        confirmLabel="Delete"
        confirmingLabel="Deleting…"
        isConfirming={pending === confirmDelete}
        tone="danger"
